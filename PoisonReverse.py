# Implements Distance Vector routing protocol with Poisoned Reverse.
        ...
        for router in routers:
            if router.name in neighbors:
                ...
                for dest, (cost, next_hop) in self.routing_table.items():
                    if next_hop == router.name and dest in table:
                        for via_node in table[dest]:
                            table[dest][via_node] = INF
